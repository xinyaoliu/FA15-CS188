...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## DESCRIBE YOUR ENHANCED FEATURES HERE...
        implementing the number of white regions 
    ##
    """
    ...
    for x in range(1, DIGIT_DATUM_WIDTH - 1):
        for y in range(1, DIGIT_DATUM_HEIGHT - 1):
            if datum.getPixel(x, y) > 0 and datum.getPixel(x, y - 1) == 0 and datum.getPixel(x - 1, y) == 0 and datum.getPixel(x - 1, y - 1) == 0:
            # if datum.getPixel(x,y) == 0 and datum.getPixel(x,y) == 0 and datum.getPixel(x,y) > 0:
                a += 1
            if datum.getPixel(x, y) == 0 and datum.getPixel(x, y + 1) > 0 and datum.getPixel(x + 1, y) > 0 and datum.getPixel(x + 1, y + 1) > 0:
            # if datum.getPixel(x,y) > 0 and datum.getPixel(x,y) > 0 and datum.getPixel(x,y) == 0:
                b += 1
    regions = a - b
    # print regions
    # np.append(features, [regions])
    features = np.append(features, [regions])
    # print features
    # print holes
    # print "end"
    # features = util.Counter()
