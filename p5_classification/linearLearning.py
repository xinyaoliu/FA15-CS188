...
def stochasticGradientDescentUpdate(datum, label, weights, alpha, der_loss_dw):
    ...
    weights. 
    Function signature: der_loss_dw(datum, label, weights), returns an array 
...
def softmax(x):
    ...
    exp_arr = np.exp(x)
    total_sum = np.sum(exp_arr)
    return np.true_divide(exp_arr, total_sum)
...
class LinearRegression:
    ...
    def trainAnalytical(self, trainingData_x, trainingData_y):
        ...
        x2 = np.dot(inv, x_T)
        weights = np.dot(x2, trainingData_y)

        

        # print inv 

        # print trainingData_y
        # util.raiseNotDefined()
    ...
    def hypothesis(self, x):
        ...
        # output the dot product of x and self.weights
        "*** YOUR CODE HERE ***"
    ...
    def loss(self, x, y_true):
        ...
        return quadLoss(y_predict, y_true)
    ...
    def der_loss_dw(self, x, y_true, weights):
        ...
        """
        "*** YOUR CODE HERE ***"
        wx = self.hypothesis(x)
        dw = der_dot_dw(x, weights)
        loss = der_quadLoss_dx(wx, y_true)

        return loss * dw

        # dw = der_dot_dw(x, weights) #der wx = x
        ...
        # # temp2 = np.dot(temp, dw)

        # # return -2*temp2
        # print "end"
        # # prod = np.dot(weights*dw)

        # # return der_quadLoss_dx(weights*dw, y_true)   #der loss/w = -2(y-wx)



        # # return [6.977552,10.5344]
        # # return der_quadLoss_dx(der_dot, y_true)
        # # util.raiseNotDefined()
...
class BinaryLinearClassifier:
    ...
    def train( self, trainingData, trainingLabels, validationData, validationLabels, showPlot=True, showPacmanPlot=True):
        ...
        numDimensions = trainingData[0].size
        # numDimensions = trainingData[0].len()
    ...
    def der_loss_dw(self, x, y_true, weights):
        ...
        # quadLoss = der_quadLoss_dx(hypo, y_true)
        # return quadLoss * (-x) * math.exp(temp) / hypo**2
        ...
        # loss = (y-1/1+e^-x)^2
        hypo = self.hypothesis(x)
        der_quadLoss = der_quadLoss_dx(hypo, y_true)
        wx = np.dot(x, weights)
        der_sigmoid = der_sigmoid_dx(wx)
        return der_quadLoss * der_sigmoid * (x)
        # return der_quadLoss * der_sigmoid * der_dot_dw(x, weights)
...
class MulticlassLinearClassifier:
    ...
    def hypothesis(self, x):
        ...
        ret = np.zeros(numClasses)
        for i in range(0, numClasses):
            k = np.dot(x, self.weights[i])
            ret[i] = k
        return softmax(ret)
